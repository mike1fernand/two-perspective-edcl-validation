...
| 2. Robust late-time channel | Best-fit component accounting; likelihood ablations; kernel/prior/local-anchor robustness; workdir-backed provenance | Partially complete: best-fit component accounting is chain-verified in `cosmology/results/tierA1_chain_component_audit.json`; ablations, robustness checks, and workdir provenance remain to run/locate | “Robust late-time Hubble-resolution channel.” |
