...
#
# Reviewer-facing wrapper for the corrected Tier-A1 late-only EDCL validation suite.
#
# This script intentionally delegates to:
#
#   cosmology/scripts/run_tiera1_lateonly_suite.py
#
# instead of maintaining a separate YAML/render/run path. That avoids divergence
# between shell, Colab, and Python validation entry points.
#
# Usage:
#   ./RUN_TIER_A_VALIDATION.sh
#       Clone, patch, build CLASS from cosmology/patches/class_edcl.patch,
#       render Tier-A1 YAMLs into the workdir, run Cobaya, validate, and bundle.
#
#   ./RUN_TIER_A_VALIDATION.sh /path/to/class_public
...
#
# Notes:
# - The Python runner creates logs, rendered YAMLs, chains, validation reports,
#   manifest.json, and bundle_edcl_tiera1.zip under the workdir.
# - Heavy outputs should not be committed to git. Publish them as Release assets
#   if they are needed for external reproducibility.
# - COBAYA_PACKAGES_PATH is not required by this wrapper; the Python runner uses
#   a workdir-local cobaya_packages directory.
...
RUNNER="$SCRIPT_DIR/cosmology/scripts/run_tiera1_lateonly_suite.py"

if [[ ! -f "$RUNNER" ]]; then
    echo "ERROR: Tier-A1 suite runner not found: $RUNNER" >&2
    exit 1
fi
...
echo "=================================================="
echo ""
echo "Wrapper: RUN_TIER_A_VALIDATION.sh"
echo "Runner:  $RUNNER"
