...
This file lives under colab/ and intentionally does not implement a separate
Tier-A workflow. It delegates to canonical scripts in the repository root so
Colab, shell, and local workflows cannot drift apart.
...
Legacy examples still supported from the repository root:
  python colab/COLAB_TIER_A_VALIDATION.py --validate-only --chains-dir ./chains
  python colab/COLAB_TIER_A_VALIDATION.py --full-run --class-path ./class_public
...
# This file is in <repo>/colab/, so repo root is one parent up.
